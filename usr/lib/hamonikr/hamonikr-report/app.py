...
class MyApplication(Gtk.Application):
    ...
    def activate(self, application):
        ...
        if (len(windows) > 0):
            ...
            window = HamonikrReportWindow(self)
            self.add_window(window.window)
            window.window.show_all()

class HamonikrReportWindow():
    ...
    def dump_crash_report(self, report):
        ...
        # Add info about the HamoniKR OS release
        if os.path.exists("/etc/hamonikr/info"):
            shutil.copyfile("/etc/hamonikr/info", "HamoniKRInfo")
                ...
                if "mate" in output or output in ["caja", "atril", "pluma", "engrampa", "eom"]:
                    ...
                    for origin in pkg.installed.origins:
                        if origin.origin == "linuxmint":
                            ...
                            break
                        if origin.origin == "hamonikr":
                            self.bugtracker = "https://github.com/hamonikr/%s/issues" % output
                            break
        except:
